window.RELAYR=function(a,b){function d(a){function f(b){function h(){d.user().getUserInfo(f,function(){b.success(f)},function(a){b&&b.error&&"function"==typeof b.error?b.error(a):(localStorage.removeItem("relayrToken"),d.login(b))})}if(!a||!e.nonEmptyString(a.redirectUri))throw new Error("Provide credentials: redirectUri");if(!b||!b.success||"function"!=typeof b.success)throw new Error("Provide the method success within your parameters");var f=localStorage.getItem("relayrToken");if(f)h();else{var g={client_id:d.credentials.appId,redirect_uri:d.credentials.redirectUri,scope:"access-own-user-info"};this.login.redirect(c.url+"oauth2/auth?client_id="+g.client_id+"&redirect_uri="+g.redirect_uri+"&response_type=token&scope="+g.scope)}}function g(){this.getUserInfo=function(a,b,c){if(d.account){if(!b||"function"!=typeof b)return d.account;b(d.account)}else e.ajax({url:d.config.url+"oauth2/user-info",type:"GET",token:"Bearer "+a,isObject:!0},function(c){c.token=a,d.account=c,b(d.account)},function(a){c(a)})}}function h(){this.getDeviceData=function(b){if(!b||!b.incomingData||"function"!=typeof b.incomingData)throw new Error("Provide the method incomingData within your parameters");if(!b||!b.deviceId)throw new Error("Provide the deviceId within your parameters");return b.token||d.account.token&&(b.token=d.account.token),b.token=b.token.replace("Bearer ",""),e.ajax({url:d.config.url+"apps/"+a.appId+"/devices/"+b.deviceId,type:"POST",token:"Bearer "+b.token,isObject:!0},function(a){var c=PUBNUB.init({ssl:!0,subscribe_key:a.subscribeKey,cipher_key:a.cipherKey,auth_key:a.authKey});c.subscribe({channel:a.channel,message:function(a){b.incomingData&&b.incomingData(JSON.parse(a))},error:function(a){b.error&&b.error(a)}})},function(){localStorage.removeItem("relayrToken"),d.login()}),this},this.getAllDevices=function(a){return e.ajax({url:d.config.url+"users/"+d.account.id+"/devices",type:"POST",token:"Bearer "+d.account.token,isObject:!0},function(b){a(b)},function(){}),this}}if(!a||!e.nonEmptyString(a.appId))throw new Error("Provide credentials: appId and redirectUri");var c={url:"https://api.relayr.io/",mockMode:!1},d=this;d.credentials=a,d.config=c,function(){function a(a){var b=a.split("#");if(b.length<2)return void 0;var c=b[1].split("&"),d=c.reduce(function(a,b){var c=b.split("=");return a[c[0]]=c[1],a},{});return d.access_token}var b=a(document.location.hash),c=window.location.href,d=c.indexOf("#access_token");d>0&&(window.location=c.substring(0,d)),b&&localStorage.setItem("relayrToken",b)}(),this.login=f,this.user=function(){return new g},this.devices=function(){return new h},this.login.redirect=function(a){b.location=a}}var c={init:function(a){return new d(a)}},e={nonEmptyString:function(a){return a&&"string"==typeof a&&a.length>0},ajax:function(a,b,c){var d=new XMLHttpRequest;d.onreadystatechange=function(){4===d.readyState&&(d.status>199&&d.status<299&&(a.isObject?b(JSON.parse(d.responseText)):b(d.responseText)),401===d.status&&c(d))},d.open(a.type,a.url,!0),d.setRequestHeader("Authorization",a.token),d.send()},loadScript:function(a,b){var c=document.getElementsByTagName("head")[0],d=document.createElement("script");d.type="text/javascript",d.src=a,d.onreadystatechange=b,d.onload=b,c.appendChild(d)}};return c}(window,window.document);
//Pubnub lib
// Version: 3.6.7
(function(){
/*
